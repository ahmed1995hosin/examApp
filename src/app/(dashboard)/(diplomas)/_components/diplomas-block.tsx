...
export default function DiplomasBlock() {
    ...
    const {  
        ...
        hasNextPage,
        error,
        isLoading}=useDiplomas();
    
    // router
    const router = useRouter();

    // get diplomas
    const diplomasAll: Diploma[] = diplomas?.pages.flatMap(page => page.subjects) || [];
    ...
    return (
           <InfiniteScroll
            ...
            hasMore={!!hasNextPage}
            next={fetchNextPage}
            loader={<p className="flex flex-col text-base items-center my-6 font-light text-gray-600 ">Loading more...</p>}
           >
                {/* loading */}
                {isLoading && (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-2.5 px-6">
                    {diplomasAll.map((diploma) => (
                        <div 
                            ...
                            <Image src={diploma.icon} alt={diploma.name} width={100} height={0} className="w-full object-cover h-full "/>
                                <div className="absolute left-2.5 bottom-2.5 right-2.5 object-fit">
                                    <h3 className="text-xl font-semibold px-4 py-5 bg-[#155DFC80] text-white text-center backdrop-blur-[6px]">{diploma.name}</h3>
                                </div> 
                        </div>
                    ))}                        
                </div>
               ...
               {/* scroll to view more */}
                {!isFetchingNextPage && !isLoading && (<div className="flex flex-col text-base items-center my-6 font-light text-gray-600">
                {hasNextPage ? (
                    <>
